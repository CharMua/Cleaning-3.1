...
{
    static int32_t temp, err_1, err_2, warn_count = 0, cnt = 0;
    char temp_id = (id == 0x181) ? '1' : '2';
    temp = rxdata[1] << 8;
    err_1 = temp + rxdata[0];
    temp = rxdata[3] << 8;
    err_2 = temp + rxdata[2];
    ...
    while (err_1)
    {
        ...
        if (cnt == 1)
        {
            ...
            strcpy(alarm_err1.sensor_id, &temp_id);
            strcpy(alarm_err1.alarm_id, "12001002");
        }
        warn_count++;
    }
    ...
    while (err_2)
    {
        ...
        if (cnt == 1)
        {
            ...
            strcpy(alarm_err2.sensor_id, &temp_id);
            strcpy(alarm_err2.alarm_id, "12001002");
        }
        warn_count++;
    }
}
...
void HAL_FDCAN_RxFifo0Callback(FDCAN_HandleTypeDef *hfdcan, uint32_t RxFifo0ITs)
{
    ...
    if ((RxFifo0ITs & FDCAN_IT_RX_FIFO0_NEW_MESSAGE) != RESET)
    {
        ...
        if (ck_lf && ck_rh)
        {
            ...
            ck_lf = false;
            ck_rh = false;
            xQueueSendFromISR(CAN_Queue, can_data, &xHigherPriorityTaskWoken); //������з�������
